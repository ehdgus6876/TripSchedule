...
public class MapActivity2 extends AppCompatActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(@NonNull NaverMap naverMap) {
        ...
        for(int i=0;i<ScheduleActivity.al.length;i++){
            List<LatLng> latLngs=new ArrayList<>();
            for(int j=1;j<ScheduleActivity.al[i].size();j++){
                marker=new Marker();
                ...
                marker.setCaptionText(ScheduleActivity.al[i].get(j).getTitle());
                ...
                Tm128 tm128 = new Tm128(Float.valueOf(ScheduleActivity.al[i].get(j).getMapx()), Float.valueOf(ScheduleActivity.al[i].get(j).getMapy()));
